...
- [x] **fix/final-home-stability-layout-flow:** порядок блоков на главной Hero→AI→Города→Фильтр→Статистика→Карточки; кнопка «Подобрать жильё» открывает AI при пустом городе; анимация появления фильтра (opacity/translateY 220ms); FilterBar hover/active; кнопка «Выйти» — прозрачный фон, красная обводка; уведомления и бронирование на var(--card-bg); дубли профиля в «Разместить» не было — оставлено как есть.
