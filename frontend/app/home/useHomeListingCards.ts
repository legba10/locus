...
export function useHomeListingCards(data: ListingsResponse | undefined) {
  return useMemo(() => (data?.items || []).map((listing: any, index: number) => {
    ...
    return {
      ...
      viewsCount: views > 0 ? views : null,
      favoritesCount: listing.favoritesCount ?? null,
      availableToday: listing.availableToday ?? false,
      availableFrom: listing.availableFrom ?? null,
      aiRecommendTooltip: (listing.reasons && listing.reasons[0]) || listing.verdict || undefined,
