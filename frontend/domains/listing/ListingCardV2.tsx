...
import { LocusQuickTip } from '@/shared/ui/locus-ai/LocusRecommendationBlock'
...
interface ListingCardV2Props {
  ...
  // AI данные
  score?: number
  verdict?: string // "Отличный вариант" и т.д.
  priceText?: string // "Ниже рынка на 8%"
  demandLevel?: 'low' | 'medium' | 'high'
  recommendation?: string // Короткая рекомендация
...
function DemandDots({ level }: { level: string }) {
  ...
  return (
    <div className="flex gap-0.5 items-center">
      {[1, 2, 3].map((d) => (
        <div key={d} className={cn('w-1.5 h-1.5 rounded-full', d <= dots ? color : 'bg-gray-200')} />
      ))}
    </div>
  )
}
...
/**
 * ListingCard 2.0 — карточка с AI-решением
 * 
 * Принцип: Показываем не просто объект, а решение
 */
export function ListingCardV2({
  ...
  return (
    <Link 
      href={`/listings/${id}`}
      className={cn(
        'group block rounded-2xl border border-gray-200 bg-white overflow-hidden',
      ...
      <div className="relative aspect-[4/3] bg-gray-100">
        ...
        {priceText && (
          <div className={cn(
            ...
            priceText.includes('ниже') ? 'bg-emerald-500 text-white' :
            priceText.includes('выше') ? 'bg-amber-500 text-white' :
...
/**
 * ListingCardV2Skeleton — скелетон загрузки
 */
export function ListingCardV2Skeleton({ className }: { className?: string }) {
  return (
